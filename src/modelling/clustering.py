...
class SSGraphKMeans(object):
    ...
    def _set_borders(self, cluster=None):
        ...
        def reset_cluster_border(cluster):
            ...
            for node in self.clusters[cluster]:
                ...
                if not all(neighbors_in_this_cluster):
                    new_border.add(node)
            self.clusters[cluster].border = new_border
    ...
    def _all_clusters_within_tolerance(self, tol):
        '''Check if all clusters are within the specified tolerance
        '''

        return all([
            self._cluster_within_tolerance(cluster_id, tol)
    ...
    def _anneal(self, cluster_id, tol):
        ...
        if self._only_frozen_neighbors(cluster_id):
            return
    ...
    def _only_frozen_neighbors(self, cluster_id):
        '''Check to see if the given cluster touches only frozen neighbors
        '''

        for border_member in self.clusters[cluster_id].border:
    ...
    def _sorted_border(self, cluster_id):
        ...
        cluster_subgraph = self.graph.subgraph(self.clusters[cluster_id])
        btwnnss = nx.betweenness_centrality(cluster_subgraph)
    ...
    def _cluster_is_connected(self, cluster_id):
        '''Determine if given cluster is connected
        '''
    ...
    def _reconnect_clusters(self, cluster_id=None):
        ...
        if cluster_id is None:
            for _cluster in self.clusters:
                while not self._cluster_is_connected(_cluster):
